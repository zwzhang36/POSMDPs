    ...
    def get_dwell_times_Poz(self, condition='agent'):
        ...
        if condition == 'agent':
            ...
            for key in D.keys():
                ...
                # D[key] = 0*D[key] + self.params.baseline.D
            return D
...
class odorTaskLesion(odorTask):
    ...
    def get_transition_matrix(self, condition='agent', baseline_big_T=1e-3):
        ...
        if self.task_space == 'poz':
            ...
            # # we don't need to this for the VS lesion
            # p = self.largerP  
            # if condition == 'agent':
...
class Agent(object):
    ...
    def dwell_update(self, o_tp1):
        ...
        for s in range(self.n_states):
            ...
            if self.lesion:
                k_distribution = 0*k_distribution
            ...
            self.D_matrix[s] = self.D_matrix[s] + lr * (k_distribution - self.D_matrix[s]) * self.beta[-1][s]
            # self.D_matrix[s] = copy.deepcopy(normalize(self.D_matrix[s], self.baseline.D))
            self.D_matrix[s][self.D_matrix[s]<self.baseline.D] = self.baseline.D 
