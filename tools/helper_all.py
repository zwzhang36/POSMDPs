...
class odorTaskLesion(odorTask):
    ...
    def get_transition_matrix(self, condition='agent', baseline_big_T=1e-3):
        ...
        if self.task_space == 'poz_rwd':
            ...
            # p = self.largerP  # 0.65
            # if condition == 'agent':
            #     n_cue, n_well = len(self.state['cue_L']), len(self.state['well_L'])
            #     for i in range(n_cue):
            #         for j in range(n_well):
            #             if i==j:
            #                 T[self.state['cue_L'][i], self.state['well_L'][j]] = p
            #                 T[self.state['cue_R'][i], self.state['well_R'][j]] = p
            #             else:
            #                 T[self.state['cue_L'][i], self.state['well_L'][j]] = (1 - p)/(n_cue-1)
            #                 T[self.state['cue_R'][i], self.state['well_R'][j]] = (1 - p)/(n_cue-1)
            return T
        if self.task_space == 'poz':
...
class Agent(object):
    ...
    def init(self):
        ...
        for s in self.task.state['start']:
            # belief[s] = 1 - belief[4:].sum() /len(self.task.state['start'])
            vinit[s, ] = .7
        var = 1e-5
    ...
    def T_lesion(self, p):
        # lesion the model in the middle
        if self.task.task_space != 'poz_rwd':
            return 
        T, state_space = self.T_matrix, self.task.state
        n_cue, n_well = len(state_space['cue_L']), len(state_space['well_L'])
        for i in range(n_cue):
            for j in range(n_well):
                if i==j:
                    T[state_space['cue_L'][i], state_space['well_L'][j]] = p
                    T[state_space['cue_R'][i], state_space['well_R'][j]] = p
                else:
                    T[state_space['cue_L'][i], state_space['well_L'][j]] = (1 - p)/(n_cue-1)
                    T[state_space['cue_R'][i], state_space['well_R'][j]] = (1 - p)/(n_cue-1)

        self.T_matrix = T

    def lr_reset(self):
        
        # self.parameters.value_lr *= np.exp(-0.5)
        self.state_value *= 0.95
        # if self.parameters.value_lr < 0.2:
        #     self.parameters.value_lr = 0.2
    ...
    def transition_update(self):
        ...
        if cue == 1:
            cue_state, waitR_state = state_space['cue_L'], state_space['well_L'] 
            R1_state, R2_state = state_space['rwd_L_1'], state_space['rwd_L_2'] 
        else:
            cue_state, waitR_state = state_space['cue_R'], state_space['well_R']
            R1_state, R2_state = state_space['rwd_R_1'], state_space['rwd_R_2']
        ...
        # prior
        likelihood = np.ones(prior.shape)
        for i in range(len(start_state)):
            wstate, r1, r2 = waitR_state[i], R1_state[i], R2_state[i]
            likelihood[i] = D[wstate][dwell_wait - 1]
            if t_r2 > t_r1:
                likelihood[i] *= T[r1, r2] * D[r2][dwell_r1 - 1] * T[r2, end_state[i]] * \
                             D[r2][dwell_r2 - 1]
            else:
                likelihood[i] *= T[r1, end_state[i]] * D[r1][dwell_r1 - 1]
